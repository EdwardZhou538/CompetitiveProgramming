...
int main() {
    ...
    for (int i = 0 ; i < n; i++) {
        ...
        cin >> direction >> x >> y;
        if (direction == 'N') {
            north_cows.push_back({x, y, 0, i});
        }
        else {
            east_cows.push_back({x, y, 0, i});
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        if (east_ptr < east_cows.size() && east_cows[east_ptr][3] == i) {
            if (east_cows[east_ptr][2] == 0) cout << "Infinity" << endl;
            else cout << east_cows[east_ptr][2] - east_cows[east_ptr][0] << endl;
            east_ptr++;
        }
        else if (north_ptr < north_cows.size() && north_cows[north_ptr][3] == i) {
            if (north_cows[north_ptr][2] == 0) cout << "Infinity" << endl;
            else cout << north_cows[north_ptr][2] - north_cows[north_ptr][1] << endl;
            north_ptr++;
        }
    }
